...
const Footer = () => {
  return (
   <section className={`${styles.flexCenter} ${styles.padding} flex-col`}>
    <div className={`${styles.flexCenter} md:flex-row flex-col mb-8 w-full`}>
      ...
      flex-wrap md:mt-0 mt-10'>
        {footerLinks.map((footerLink) => (
          ...
          min-w-[150px]'>
            <h4 className='font-poppins font-normal text-[18px] leading-[24px] text-white'>
              {footerLink.title}</h4>
    ...
    <div className='flex flex-row justify-between items-center md:flex-row flex-col
    pt-6 border-t-[1px] border-t-[#3f3E45]'>
      <p className='font-poppins font-semibold text-white text-[16px] leading-[27px]'>
        2022 Ajithdev C . All Rights Reserved.
      </p>
      <div className='flex flex-row md:mt-0 mt-6'>
        {socialMedia.map((socialmedia, index) => (
          <img key={socialmedia.id}
